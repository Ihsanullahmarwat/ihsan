  ...
  // departed orders 
    ...
    if(isset($_GET['deactive']) && $_GET['deactive']=='inactive' ) {
		...
		if(mysqli_query($conn, $queryyyy )){
			...
			echo "<script> alert ('Order  activated Successfully!...'); </script>";
...
function print_this_page() {
 window.print();
								...
								<div style="font-family: paprus  ; position:fixed; width:100% ; z-index:1 ;  " class="btn-primary">
	...
	$select_from = mysqli_query($conn2, "select   menu.item_id ,  menu.item_name,  menu.hotel_id, menu.price  , order_items.c_id  , order_items.O_id, c.c_name
		...
		*/
		...
		/* Association table data */
			...
			$select_from312 = mysqli_query($conn2, "select   client.c_id ,  client.c_name  , order_items.o_id ,order_items.status , order_items.item_id , order_items.date ,order_items.hotel_id
										
										from client join order_items
										
										on client.c_id = order_items.c_id  where hotel_id=$hotel85 and status=0 GROUP BY o_id
...
$rows33333 = mysqli_num_rows($select_from312);
    ...
    <?php if($rows33333 != 0 ) { ?>
     ...
     <button class="col-md-12 col-md-12-2 increase_height_4_responsitivity ; flash ; label " style="outline:none ; border-radius: 10px 10px 0 0 ; background-color:olive ;  margin-bottom: 20px ; margin-top: 20px ">
       <h5>
          <strong>
            <p style="font-family:	Papyrus"> <span style="font-size:25px;">O</span>ld&nbsp;   <span style="font-size:30px;">o</span>rders 
	...
	<table class="table table-responsive" style="background-color:rgba(115,114,222,0.6) ; font-family: serif ; color:white ; border-radius:10px;">
	 ...
	 <td> <a class="btn btn-danger col-md-12" href="old_orders.php?can=cancel&id=<?php echo $rows['o_id']; ?>">Cancel ?</a></td>
     <td> <a class="btn btn-warning col-md-12" href="old_orders.php?deactive=inactive&id=<?php echo $rows['o_id']; ?>">Activate</a></td>
	...
	<?php } else {?>
     ...
     <button class="col-md-12 col-md-12-2 increase_height_4_responsitivity ; flash ; label " style="outline:none ; border-radius: 10px  ; background-color:olive ;  margin-bottom: 20px ; margin-top: 20px ">
       <h5>
          <strong>
            <p style="font-family:	Papyrus"> <span style="font-size:25px;">N</span>o &nbsp; <span style="font-size:25px;">O</span>ld &nbsp;   <span style="font-size:30px;">o</span>rders 
